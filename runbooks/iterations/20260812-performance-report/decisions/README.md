...
| :--- | :--- | :--- | :--- |
| **Official 200KB Benchmark** (`benchmark_1k_200kb.py`) | **35.66 RPS** | **37.60 RPS** | **+5.4% Throughput Increase** |
| **Concurrent 200KB Traffic** (100 parallel workers) | **52.73 RPS** | **64.62 RPS (12.62 MB/s)** | **+22.6% Throughput Increase** |
| **Standard Event Ingestion** (5,000 requests) | **~40 RPS (with P99 250ms+ stalls)** | **134.84 RPS (P50: 217ms)** | **+237.1% Throughput Increase** |
...
| **Ingestion Request Success Rate** | **Dropped requests / Timeouts** | **100% (5,000/5,000 accepted)** | **Zero Data Drop / 100% Success** |
...
| [**006**](file:///home/btpl-lap-22/live/messaging-pipeline/runbooks/iterations/20260812-performance-report/decisions/20260812-006-compact-bytebuffer-binary-serde.md) | Kotlin Stream Repartition Serde | Pipe-delimited string formatting (`"${eventId}\|${eventType}..."`) causing GC object churn and field corruption on `\|` in payloads | Replaced with compact `ByteBuffer` binary framing for corruption-free deserialization |
