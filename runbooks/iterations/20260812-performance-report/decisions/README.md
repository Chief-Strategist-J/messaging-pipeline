...
| [**009**](file:///home/btpl-lap-22/live/messaging-pipeline/runbooks/iterations/20260812-performance-report/decisions/20260812-009-true-fire-and-forget-kafka-produce.md) | Fire-and-Forget Kafka Produce | Synchronous `errChan` channel block on every HTTP goroutine waiting for Kafka network ACK | True non-blocking fire-and-forget produce; delivery errors handled asynchronously in kgo callback with OTEL span tracing |
| [**010**](file:///home/btpl-lap-22/live/messaging-pipeline/runbooks/iterations/20260812-performance-report/decisions/20260812-010-in-memory-lru-coarse-dedup-and-fail-open-resilience.md) | Edge Deduplication & SPOF Removal | Synchronous `SetNX` calls to Redis per HTTP request causing network RTT amplification and 503 outages on Redis failovers | Added in-memory `sync.Map` coarse filter and fail-open resilience; Redis glitches fail open to return 202 Accepted |
| [**011**](file:///home/btpl-lap-22/live/messaging-pipeline/runbooks/iterations/20260812-performance-report/decisions/20260812-011-zero-allocation-evaluation-context-and-bounded-body-reader.md) | GC Heap & Context Pooling | Fresh `EvaluationContext` struct + `map` allocations per request causing GC STW pauses and RWMutex cache line bouncing | Added `sync.Pool` for `EvaluationContext`, 0-lock metadata lookups, and bounded `http.MaxBytesReader` |
| [**012**](file:///home/btpl-lap-22/live/messaging-pipeline/runbooks/iterations/20260812-performance-report/decisions/20260812-012-decoupled-edge-in-memory-filtering-and-asynchronous-stream-deduplication.md) | Edge In-Memory & Stream Dedup | Redundant double deduplication paying Redis network IO tax at edge and RocksDB write tax in stream processor | Zero-network in-memory edge coarse filter; exact stateful windowed deduplication deferred to downstream stream processor |
